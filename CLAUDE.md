...
`View/World3DWindow.xaml(.cs)` is an **interactive** DX11 fly-through of every placed actor, opened **non-modally** from the menu (`Menu3DView` → `MainViewModel.Open3DViewCommand` → `new World3DWindow(saveGame.Entries).Show()`). It uses **`HelixToolkit.Wpf.SharpDX` 3.x** (DX11; renders fine over RDP). **By design it *does* mutate the save** — left-click selects (synced to the editor tree/right-pane via `MainViewModel.SelectByInstanceName` + an in-window detail panel), and `Delete` / `Ctrl+D` delete/duplicate through `MainViewModel.DeleteByInstanceName` / `DuplicateByInstanceName`. This was an explicit product request; it is **not** a read-only window, so don't "fix" the editing back out. Mutations still go through the Model layer (never the raw parser object), so the round-trip rule holds. Gotchas, all load-bearing:
...
- Camera is **placed explicitly from the scene bounds** (not `ZoomExtents`). All actors render as one **merged cube `MeshGeometryModel3D`** (vertex-colored per category, colors mirror `MapRender`) — cube ordinal `i` owns verts `[i*8..]`/indices `[i*36..]`, so a `FindHits` triangle index `/8` reverse-maps to the `SaveEntity` for picking (HitTestResult carries no instance index, so instancing can't be picked — the merged mesh is the only selection-preserving option). Structural (cat-9) objects render at **real footprint** (`SizeOfGame` class→size resolver × `Scale`, rotated by the actor quaternion in game space before the `(X,Z,Y)` swap). A **read-only connection overlay** draws belt/pipe `mSplineData` (local→world) and PowerLine `mWireInstances` (absolute world) as three merged `LineGeometryModel3D` (one per category, `IsHitTestVisible=false` so they don't perturb the cube-pick ordinal); decoded via the existing `PropertyTypes/V2` reader without touching `RawData`. **Built: M1 point cloud → M2 selection-sync + detail panel + delete/duplicate + foundations + connections. Pending: XYZ move gizmo and new-object placement.** Adding a *new* 1.0 object via the editor is intentionally unsupported at save time — `SaveBodyV2.BuildData` throws `NotSupportedException` for a 1.0 object with null `RawData` rather than silently writing a corrupt empty frame.
